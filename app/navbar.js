    ...
    fetch('http://localhost:5000/profile', {
        ...
        .then(data => {
            if (data.success) {
                ...
                localStorage.setItem('userEmail', data.user.email);  // Save to localStorage if needed
            } else {
...
useEffect(() => {
  ...
  // Clear the timeout if the component unmounts or query changes before debounce completes
  return () => clearTimeout(debounceTimeout);
}, [query]);

  return (
    <nav
    className="block w-full  px-4 py-2 pr-5 mx-auto text-black bg-white border shadow-md rounded-xl border-white/80 bg-opacity-80 backdrop-blur-2xl backdrop-saturate-200 lg:px-8 lg:py-4">
    <div className="container flex flex-wrap items-center justify-between mx-auto text-blue-gray-900">
      <a href="#"
        className="mr-4 text-xl block cursor-pointer py-1.5 pl-4 font-sans text-base font-medium leading-relaxed text-inherit antialiased">
       Watch Shree
      </a>
      <div className="hidden lg:block">
        <ul className="flex flex-col gap-2 mt-2 mb-4 ml-20 lg:mb-0 lg:mt-0 lg:flex-row lg:items-center lg:gap-6">
          <li
      ...
      <div className="hidden gap-x-2 lg:flex flex-col">
        <div className='flex gap-x-4 items-center'>

     
        <div className="relative flex w-full gap-2 md:w-max flex-col">
    ...
    <a href='/profile'><FontAwesomeIcon icon="fa-solid fa-user" size="lg" className='text-gray-700'/></a>  
       <a href='/cart'>
       <FontAwesomeIcon icon="fa-solid fa-bag-shopping" size='lg' className='text-gray-700'/>
       </a>
        {/* <button
...
(query.length > 0) && (
    <ul style={{
        boxShadow: '0px 3px 7px 2px #00000069',
        width: '74%',
        marginLeft: '2px',
